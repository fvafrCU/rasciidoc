...
if (interactive()) {
    ...
    # base's system.file
    unit_dir <- system.file("inst", "runit_tests", package = "asciidoc")
} else {
    require("asciidoc", quietly = TRUE, character.only = TRUE) ||
    ...
    if (any(grepl("--file", r_call))) {
        ...
        unit_dir <- system.file("runit_tests", package = "asciidoc")
    }
}
...
package_suite <- RUnit::defineTestSuite("asciidoc_unit_test",
                                        dirs = unit_dir,
                                        testFileRegexp = "^.*\\.[rR]",
